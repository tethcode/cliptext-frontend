...
export default function ProfilePage() {
  ...
  const removePhoto = async () => {
    ...
    // This tells Django to clear the ImageField
    ...
    try {
      ...
      // Update local state to show the letter avatar immediately
  ...
  if (!user) return <div className="animate-pulse text-zinc-500">Loading profile...</div>;
  ...
  return (
    <div className="mx-auto max-w-4xl font-sans">
      <header className="mb-12">
        ...
        <p className="mt-2 text-zinc-500">Update your personal information and profile picture.</p>
      </header>

      <form onSubmit={handleUpdate} className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="rounded-3xl border border-white/10 bg-zinc-900/50 p-8 text-center backdrop-blur-xl h-fit">
          <div className="group relative mx-auto mb-6 h-32 w-32">
            <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-3xl bg-blue-500/20 border border-blue-500/30 text-4xl font-bold text-blue-500 shadow-2xl shadow-blue-500/10">
            ...
            <button 
              ...
              className="absolute inset-0 flex items-center justify-center rounded-3xl bg-black/60 opacity-0 transition-opacity group-hover:opacity-100 backdrop-blur-sm"
            >
          ...
          <div className="flex flex-col gap-3 items-center">
             <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Click to change</p>
             {(user?.profile_pic || previewUrl) && (
               <button 
                type="button" 
                onClick={removePhoto}
                className="flex items-center gap-1 text-[11px] text-red-400 hover:text-red-300 transition-colors font-semibold bg-red-500/10 px-3 py-1 rounded-full border border-red-500/20"
               >
          ...
          <div className="mt-8 flex items-center gap-2 rounded-2xl bg-white/[0.03] p-3 text-[11px] text-zinc-400 border border-white/5">
            <HiOutlineBadgeCheck className="text-blue-500 text-lg" />
            <span className="truncate">Verified Account</span>
          </div>
        </div>

        <div className="lg:col-span-2 space-y-6 rounded-3xl border border-white/10 bg-zinc-900/30 p-8">
          {status && (
          ...
          <div className="space-y-5">
            <div className="group">
              ...
              <div className="mt-2 relative">
                ...
                <input 
                  className="w-full rounded-2xl border border-white/5 bg-white/5 p-4 pl-12 text-white outline-none focus:border-blue-500/40 focus:bg-white/[0.07] transition-all"
